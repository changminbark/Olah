...
const PostWidget = ({
  ...
  const patchUsers = async () => {
    const response = await fetch(
      `http://localhost:3001/posts/${postId}/users`,
      {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    <WidgetWrapper m="2rem 0">
      ...
      {locationPicturePath && (
        <img
          ...
          src={`http://localhost:3001/assets/${locationPicturePath}`}
        />
      )}
      <FlexBetween mt="0.25rem">
        <FlexBetween gap="1rem">
          ...
          <FlexBetween gap="0.3rem">
            <IconButton onClick={patchUsers}>
              {isJoined ? (
                <AccountCircleOutlined sx={{ color: primary }} />
              ) : (
                <AccountCircle />
              )}
            </IconButton>
            <Typography>{joinCount}</Typography>
          </FlexBetween>
        </FlexBetween>
